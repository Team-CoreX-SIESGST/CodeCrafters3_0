...
# ---------------------------------------------------------------------------
# MongoDB store — stores EVERY crucial event in its own collection
# ---------------------------------------------------------------------------
class MongoStore:
    """
    Collections stored in MongoDB (and mirrored in memory when DB unavailable):

    cognitive_snapshots       — every 3-s composite snapshot
    cognitive_events          — human-readable event log
    cognitive_features_raw    — raw feature vectors (for ML training later)
    cognitive_state_changes   — only when state transitions happen
    cognitive_z_scores        — z-score history (for drift analysis)
    recovery_capsules         — task recovery capsules + blocker notes
    interruption_batches      — queued interruption summaries
    artifact_friction         — per-artifact friction scores + visit counts
    cognitive_twins           — baseline / personalisation metadata
    attention_residue_events  — every residue spike > 0.5
    pre_error_events          — every pre-error risk spike > 0.6
    fatigue_events            — every fatigue risk spike > 0.6
    confusion_episodes        — confusion episodes (start / ongoing / resolved)
    handoff_capsules          — proactive handoff snapshots on idle
    """
    ...
    def __init__(self) -> None:
        ...
        try:
            ...
            # Attach collection references
            for bucket in self.BUCKETS:
                setattr(self, bucket, db[f"cognitive_{bucket}"])

            # Indexes for efficient querying
            self.snapshots.create_index(
                [("user_id", DESCENDING), ("generated_at", DESCENDING)]
            )
            self.snapshots.create_index([("state_label", ASCENDING)])
            self.state_changes.create_index(
                [("user_id", DESCENDING), ("changed_at", DESCENDING)]
            )
            self.attention_residue_events.create_index(
                [("user_id", DESCENDING), ("created_at", DESCENDING)]
            )
            self.pre_error_events.create_index(
                [("user_id", DESCENDING), ("created_at", DESCENDING)]
            )
            self.fatigue_events.create_index(
                [("user_id", DESCENDING), ("created_at", DESCENDING)]
            )
            self.artifacts.create_index(
                [("user_id", DESCENDING), ("artifact_id", ASCENDING)]
            )
            self.confusion_episodes.create_index(
                [("user_id", DESCENDING), ("started_at", DESCENDING)]
            )
            self.enabled = True
    ...
    def team_rollup(self) -> dict[str, int | float]:
        ...
        if not members:
            return {
                "team_focus_health": 0,
                "members_in_deep_work": 0,
                "members_high_confusion": 0,
                "member_count": 0,
            }
...
class ActivityMonitor:
    def __init__(self) -> None:
        ...
        self._artifact_stats: dict[str, dict[str, float | int]] = defaultdict(
            lambda: {"visits": 0, "revisits": 0, "friction_score": 0.0, "first_seen": utcnow().isoformat()}
        )
        ...
        # State tracking for change events
    ...
    # ------------------------------------------------------------------
    # Core snapshot — called every `snapshot_interval_seconds`
    # ------------------------------------------------------------------
    def snapshot(self) -> dict[str, Any]:
        with self._snapshot_lock:
            ...
            # ---- Idle ratio (merged keyboard + mouse timestamps) ---------
            ...
            perclos         = float(features.perclos) if features.perclos is not None else 0.0

            # ---- Time-of-day contextual modifier -------------------------
            tod_modifier  = time_modifier(now)
            cal_pressure  = calendar_pressure(system.get("open_apps", []))

            # ==============================================================
            # SCORE CALCULATIONS
            # All scores are in [0.0, 1.0] and grounded in the z-score
            # classification result where possible.
            # ==============================================================

            # -- Attention Residue -----------------------------------------
            # Rises sharply after a forced switch; decays with session continuity
            residue_base   = 0.08
            ...
            # -- Pre-Error Risk --------------------------------------------
            # Driven by IKI entropy, typing speed variance, backspaces,
            # cursor entropy, and session-fatigue curve
            session_fatigue_curve = clamp(session_age / 120.0)   # peaks at 120 min
            pre_error_risk = clamp(
                0.10
                + clamp(backspaces / 12.0,           0, 0.22)
                + clamp(typing_speed_variance / 150_000.0, 0, 0.18)
            ...
            # -- Focus Depth -----------------------------------------------
            # Inversely related to risk scores; boosted by steady cursor
            focus_depth = clamp(
                0.80
                - pre_error_risk      * 0.30
            ...
            # -- Cognitive Debt (session accumulator) ----------------------
            self._debt = clamp(
                self._debt * 0.94
                + confusion_risk    * 0.06
            ...
            # ==============================================================
            # THRESHOLD-BASED CRUCIAL EVENT STORAGE
            # ==============================================================
            self._store_threshold_events(
                now             = now,
                attention_residue = attention_residue,
                pre_error_risk  = pre_error_risk,
            ...
            scores_payload = {
                ...
                "emotional_load":          round(frustration,       3),
            ...
            snapshot: dict[str, Any] = {
                ...
                "core_features": {
                    ...
                    "confusion_localization": {
                        "artifact_id":     artifact_id,
                        "artifact_label":  f"{active_app} :: {active_window}",
                        "artifact_friction": stats["friction_score"],
                        "revisit_count":   int(stats["revisits"]),
                    },
                    "cognitive_twin": {
                        "baseline_ready":              classification.baseline_ready,
                        "baseline_samples":            classification.baseline_samples,
                    ...
                    "contextual_enrichment_layer": {
                        "time_of_day_modifier": tod_modifier,
                        "session_age_minutes":  round(session_age, 1),
                        "calendar_pressure":    cal_pressure,
                    },
                    "emotional_load_detector": {
                        "frustration_index":  round(frustration,       3),
                        "negative_hits":      negative_hits,
                        "uncertainty_hits":   uncertainty_hits,
                        "iki_entropy":        round(iki_entropy, 3),
                    },
                    "camera_attention_signal": {
                        "perclos":       round(perclos, 3),
                        "status":        str(camera.get("status", "unavailable")),
                ...
                "add_ons": {
                    ...
                    "camera_fatigue_boost": round(clamp(perclos * 1.8), 3),
                },
            }

            # ==============================================================
            # STORE CORE SNAPSHOT TO DB
            # ==============================================================
    ...
    def _emit_recovery_capsule(
        self, active_app: str, active_window: str, artifact_id: str, now: datetime
    ) -> None:
    ...
    def _emit_handoff_capsule(
        self,
        active_app: str,
        active_window: str,
        artifact_id: str,
        now: datetime,
        focus_forecast: float,
    ) -> None:
        # Only fire once per idle event (don't spam on every snapshot)
    ...
    def _flush_interruption_queue(self, now: datetime) -> None:
        ...
        self.store.insert("interruptions", {
            ...
            "created_at": now,
            "summary":    summary,
            "count":      len(self._pending_interruptions),
            "items":      list(self._pending_interruptions),
        })
        self.record_event(
            f"Interruption batch delivered: {len(self._pending_interruptions)} held items."
        )
        self._pending_interruptions.clear()

    def _store_threshold_events(
        self,
        *,
        now: datetime,
        attention_residue: float,
        pre_error_risk: float,
        fatigue_risk: float,
        ...
        if IN_CONFUSION and episode_id is None:
            ...
            for ep in reversed(self.store.memory["confusion_episodes"]):
                if ep.get("episode_id") == episode_id:
                    ...
                    break
            self.record_event(
                f"Confusion episode resolved after {int(duration)}s."
            )
...
# ---------------------------------------------------------------------------
def _cursor_state(cursor: dict[str, Any]) -> str:
    speed     = float(cursor.get("cursor_speed", 0.0))
    linearity = float(cursor.get("path_linearity", 0.0))
    clicks    = int(cursor.get("click_count", 0))
    if speed >= 1_200 and linearity < 0.45:
        return "in_a_hurry"
    if linearity < 0.34 or clicks >= 4:
        return "searching"
    return "steady"


def _cursor_entropy(cursor: dict[str, Any]) -> float:
    linearity     = float(cursor.get("path_linearity", 0.0))
...
def _scroll_reversal_proxy(cursor: dict[str, Any]) -> int:
...
def _typing_speed_variance(keyboard: dict[str, Any]) -> float:
    kpm     = float(keyboard.get("keys_per_minute", 0.0))
    iki_std = float(keyboard.get("iki_std", 0.0))
    return round((kpm * max(iki_std, 0.01) * 4.0) ** 2, 3)


def _iki_entropy(keyboard: dict[str, Any]) -> float:
    iki_mean = float(keyboard.get("iki_mean", 0.0))
    iki_std  = float(keyboard.get("iki_std",  0.0))
    return round(clamp(iki_std / max(iki_mean + 0.05, 0.05)), 3)


def _frustration_index(keyboard: dict[str, Any], iki_entropy: float) -> float:
    backspaces     = float(keyboard.get("backspace_count", 0))
...
def _emotion_hits(keyboard: dict[str, Any], frustration: float) -> tuple[int, int]:
...
def _current_goal(active_app: str, active_window: str) -> str:
...
def _next_step(active_app: str) -> str:
    app = active_app.lower()
...
def _camera_disabled_snapshot() -> dict[str, Any]:
    return {
        "perclos":          None,
        "status":           "disabled",
...
def time_modifier(now: datetime) -> float:
    """
    Returns a fatigue amplifier based on circadian rhythm research.
    Higher = more vulnerable to fatigue / confusion.
    """
    hour = now.astimezone().hour
    if 13 <= hour <= 15:   return 0.55   # post-lunch dip
    if hour >= 21 or hour <= 6: return 0.65   # late night / early morning
    if 10 <= hour <= 12:   return 0.18   # morning peak
    return 0.32


def calendar_pressure(open_apps: list[dict[str, Any]]) -> float:
    hits = 0
...
def _open_comm_apps(open_apps: list[dict[str, Any]]) -> list[str]:
    results = []
    for app in open_apps:
        haystack = f"{app.get('name', '')} {app.get('title', '')}".lower()
...
def create_api_app(monitor: ActivityMonitor) -> FastAPI:
    app = FastAPI(title="Flow Guardian Cognitive API", version="1.0.0")
    app.add_middleware(
        CORSMiddleware,
        allow_origins=["*"],
    ...
    def health():
        return {
            "ok":            True,
            "mongo_enabled": monitor.store.enabled,
            "mongo_error":   monitor.store.error_message,
